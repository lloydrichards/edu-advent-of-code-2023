...
const isntOfAKind = (number: number) =>
  flow(
    ...
    E.fromPredicate((x) => Object.keys(x).includes("J"), identity),
    E.fold(flow(R.some((n) => n === number)), (hand) =>
      pipe(
        hand,
        R.deleteAt("J"),
        R.some((n) => n + hand["J"] == number),
      ),
    ),
    (x) => !x,
  );
...
const isntFullHouse = flow(
  ...
  E.fromPredicate((x) => Object.keys(x).includes("J"), identity),
  E.fold(
    flow(Object.values, (x) => !(x.includes(2) && x.includes(3))),
    flow(
      R.filter((n) => n == 2),
      R.size,
      (x) => x !== 2,
...
const scoreHand = ({ hand, bid }: { hand: string; bid: number }) =>
  pipe(
    ...
    E.chain(E.fromPredicate(isntOfAKind(5), () => HandType.FiveOfAKind)),
